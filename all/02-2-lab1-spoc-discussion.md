...
- 初始化物理内存管理单元
- 初始化8259A中断控制器
- 初始化IDT：系统调用中断使用陷阱门描述符，其它中断使用中断门描述符。
- 初始化8253时钟外设
...
- 先实现一个可以显示字符串的bootloader
  1. 初始化寄存器
  1. 开启A20
  1. 初始化GDT表
  1. 从实模式切换至保护模式
- 再实现bootloader对elf的读取和执行功能

> 


如何能获取一个系统调用的调用次数信息？如何可以获取所有系统调用的调用次数信息？请简要说明可能的思路。(spoc)
- 由于每个系统调用都对应一个系统调用编号，而系统通过一个统一的包含有系统调用编号参数的函数来实现所有的系统调用，
因此可以维护一个全局的数组记录每个系统调用被调用的次数，在每次执行统一的系统调用函数时根据对应的系统调用编号对该数组进行更新即可。
